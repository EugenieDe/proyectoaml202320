    ...
    *,
    ...
    if aspect_ratio_grouping:
        ...
        if collate_fn is None:
            return data_loader
        return MapDataset(data_loader, collate_fn, mode=mode)
    else:
...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None, mode=None):
    assert mode in ["train_l", "train_u"], f"Mode is {mode}, should be train_u or train_l."
    if dataset is None:
        if mode == "train_u":
            dataset = get_detection_dataset_dicts(
                cfg.DATASETS.TRAIN_U,
                filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
                min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
                if cfg.MODEL.KEYPOINT_ON
                else 0,
            ...
            _log_api_usage("dataset." + cfg.DATASETS.TRAIN_U[0])
        elif mode == "train_l":
            dataset = get_detection_dataset_dicts(
            ...
            _log_api_usage("dataset." + cfg.DATASETS.TRAIN_L[0])
    if mapper is None:
